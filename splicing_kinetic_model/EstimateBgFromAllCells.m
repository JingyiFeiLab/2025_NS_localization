...
for i=1:seriesCount
    ...
    I_raw(:,:,3) = bfGetPlane(reader, 1);
    I_raw(:,:,2) = bfGetPlane(reader, 2);
    I_raw(:,:,1) = bfGetPlane(reader, 3);

    pathTomask=[allcells_folder filesep 'ad-' wellname '_' num2str(i) '_cp_masks.png'];
    ...
    for j=1:cell_num_image
        ...
        cell_raw_ins(cell_id_all,4)=sum(sum(uint16(bw_cell_crop).*I_crop(:,:,1)));
        cell_raw_ins(cell_id_all,5)=sum(sum(uint16(bw_cell_crop).*I_crop(:,:,2)));
        A_cell=sum(sum(uint16(bw_cell_crop)));
        cell_raw_ins(cell_id_all,12)=cell_raw_ins(cell_id_all,4)./A_cell;
        cell_raw_ins(cell_id_all,13)=cell_raw_ins(cell_id_all,5)./A_cell;
...
save([save_folder filesep wellcons 'cell_raw_ins_allcells.mat'],'cell_raw_ins','bg_info')
bg_R_fromallcells=median(cell_raw_ins(:,12));
bg_G_fromallcells=median(cell_raw_ins(:,13));
save([save_folder filesep wellcons 'bg_from_allcells.mat'],'bg_R_fromallcells','bg_G_fromallcells')