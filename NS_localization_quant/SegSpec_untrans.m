%% SegSpec_untrans.m
% The untransfected cells are only used for background calculation,
% so only 100 cells are selected as a sample for this analysis.
%%
% Load data from previous steps
load([nucleusname filesep consname 'nu_raw.mat'])
load([untransname filesep consname '_untrans.mat'])
untrans_id=untrans(:,3);
untrans_num=length(untrans_id);
...
spec_intensity=zeros(untrans_num,6);
...
for i=1:untrans_num
    ...
    if speckleprop.N < minnum_B
        speckleprop=[];
        bw_spec=[];
        save([spec_untrans_name filesep 'speckle_', num2str(i), '.mat'],'speckleprop','bw_spec');
        continue
    end
    ...
    if j<10
        ...
        saveas(gcf,[spec_untrans_name filesep num2str(j) '_specB.png'])
        figure
        imshow(I_raw_mag)
        hold on
        visboundaries(bw_spec_mag,'LineWidth',0.5,'LineStyle',':')
        saveas(gcf,[spec_untrans_name filesep num2str(j) '_specRGB.png'])
        imwrite(I_crop,[spec_untrans_name filesep num2str(j) '.tif'])
        imwrite(bw_spec,[spec_untrans_name filesep num2str(j) '_spec_bw.png'])
...
save([spec_untrans_name filesep consname 'spec_intensity.mat'],'spec_intensity','spec_prop')
